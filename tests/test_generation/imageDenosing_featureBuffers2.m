function imageDenosing_featureBuffers2(file_name, f, r, k_c, k_f, t) 
    %% Read input
    ...
    for dx = -r:r
        for dy= -r:r
            ...
            dist_normal = ((dat_normal - ngb_normal).^2)....
                ./ (k_f^2 * max(t, sqrd_grad_normal) ) ;
            
            dist_albedo = ((dat_albedo - ngb_albedo).^2 ) ...
                ./ (k_f^2 * max(t, sqrd_grad_albedo) );
                   
            dist_depth = ((dat_depth - ngb_depth).^2) ...
                ./ (k_f^2 * max(t, sqrd_grad_depth) );
    ...
    flt = flt ./ wgtsum;
    exrwrite(flt, strcat(file_name, '_denoised_fb2_r=', int2str(r), '.exr'));
end
